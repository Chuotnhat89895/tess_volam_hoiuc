...
入坣：	pNPC: 指坑NPC的指针，nMagicAcive：打开的隝藝属性数目
出坣:	魔法被应用。
		具体工作由KNpc的戝员函数完戝。
		KItem 对象本身没有戝员坘針被修改
******************************************************************************/
...
void KItem::RemoveMagicAttribFromNPC(IN KNpc *pNPC, IN int nMagicActive /* = 0 */) const
{
	...
	for (i = 0; i < sizeof(m_aryMagicAttrib) / sizeof(m_aryMagicAttrib[0]); i++)
	{
		...
		if (INVALID_ATTRIB != pAttrib->nAttribType) // TODO: 为 -1 定义一个常針?
		{
			KItemNormalAttrib RemoveAttrib;
			if (i & 1) // 为奇数，是坎缀（i从零开始）
			{
...
void KItem::ApplyHiddenMagicAttribToNPC(IN KNpc *pNPC, IN int nMagicActive) const
{
	...
	const KItemNormalAttrib *pAttrib;
	pAttrib = &(m_aryMagicAttrib[(nMagicActive << 1) - 1]); // 坎缀为隝藝属性所以乘2凝一
...
/******************************************************************************
功能:	将item上的第N项隝藝魔法属性从NPC身上移除
入坣：	pNPC: 指坑NPC的指针，nMagicActive：第n项魔法属性
出坣:	魔法被移除。
		具体工作由KNpc的戝员函数完戝。
		KItem 对象本身没有戝员坘針被修改
******************************************************************************/
...
/******************************************************************************
功能:	根杮酝置文件中的数杮,为item的坄项赋初值
入坣：	pData: 给出来自酝置文件的数杮
出坣:	戝功时返回非零, 以下戝员坘針被值:
...
{
	if (m_nCurrentDur == -1 || nRandRange == 0) // 永丝磨束
		return -1;
...
void KItem::GetDesc(char *pszMsg, bool bShowPrice, int nPriceScale, int nActiveAttrib, bool bActiveDes)
{
	...
	if (bShowPrice)
	{
		...
		if (this->m_nPriceGold > 0)
		{
			...
			sprintf(szPrice1, "<color=Green>Gi� tr�: %d Xu", m_nPriceGold);
