...
void cLittleMap::RenderTexture()
{
	...
	rd.rcSrc = CRect(m_ptLoad.x, m_ptLoad.y, m_ptLoad.x + max_littmap_width, m_ptLoad.y + max_littmap_height);
	rd.ptDes.x = 0;
	rd.ptDes.y = 0;
...
BOOL cLittleMap::VerifyTexture(BOOL bForceUpdate)
{
	...
	CRect rcGround = CRect(m_rcRegion.left * c_nWidthRegion, m_rcRegion.top * c_nHeightRegion, m_rcRegion.right * c_nWidthRegion, m_rcRegion.bottom * c_nHeightRegion);
	CRect rcImage = CRect(m_rcRegion.left * GetImageSize(), m_rcRegion.top * GetImageSize(), m_rcRegion.right * GetImageSize(), m_rcRegion.bottom * GetImageSize());
	...
	if (!bUpdate)
	{
		if (m_rcImage.left < m_ptLoad.x || m_rcImage.right > m_ptLoad.x + c_nTextureWidth || m_rcImage.top < m_ptLoad.y || m_rcImage.bottom > m_ptLoad.y + c_nTextureHeight)
		{
			bUpdate = TRUE;
		}
	}
	...
	if (bUpdate || bForceUpdate)
	{
		...
		// center
		stLittemap &m_param = *GetMapParam();
		CRect rc(m_param.ptOffset.x, m_param.ptOffset.y, m_param.ptOffset.x + m_param.wScreen, m_param.ptOffset.y + m_param.hScreen);
...
HRESULT cLittleMap::DrawTexture()
{
	...
	if (VerifyTexture())
	{
		CRect rc(m_stMap.ptOffset.x, m_stMap.ptOffset.y, m_stMap.ptOffset.x + m_stMap.wScreen, m_stMap.ptOffset.y + m_stMap.hScreen);
